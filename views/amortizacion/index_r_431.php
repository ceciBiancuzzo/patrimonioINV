    ...
    $gridColumns = [
        ...
        ['attribute'=>'id_rubro','label'=>'Rubro','value'=>function($modelBienes){
            $modelRubro = PatrimonioRubro::find()->where(['id'=>$modelBienes->id_rubro])
                                                ->all();
            if($modelRubro[0]->codigo_presupuestario==431){
        ...
        ['attribute'=>'nro_inventario','label'=>'Nro de inventario','value'=>function($modelBienes){
            if ($modelBienes->nro_inventario) {
                return $modelBienes->nro_inventario;
            }else {
                return 0;
            }
     ...
     ['attribute'=>'precio_origen','label'=>'Precio Origen','value'=>function($modelBienes){
          if ($modelBienes->precio_origen) {
            return $modelBienes->precio_origen;
          }else {
              return 0;
          }
       ...
       'value'=>function($modelBienes){
        /* $suma= true; */
           if($modelBienes->amortizacion_anual_acumulada){
            /* if ($modelBienes->amortizacion_anual_acumulada==1) {
                $suma = false;
            } */
        ...
        ['attribute'=>'valor_residual','label'=>'Valor Residual','value'=>function($modelBienes){
           ...
           /*  function($modelBienes){
            if ($modelBienes->valor_residual>1) {
                return true;
            }else {
                return false;
            } */
        ...
        <?= GridView::widget([
            ...
            'pager' => [
               ...
               'exportConfig' => [
                GridView::CSV => [
                    'label' => 'CSV',
                    'filename' => 'Amortizacion'." - "."431",
                    'options' => ['title' => 'Amortizacion'." - "."431"],
                ],
                GridView::EXCEL => [
                    'label' => 'Excel',
                    'filename' => 'Amortizacion'." - "."431",
